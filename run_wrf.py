...
def run_wrf(wrf_root, wps_root, config, args):
	...
	expected_files = ['wrfout_d{:02d}:{}'.format(i + 1, end_time.format('YYYY-MM-DD_HH:mm:ss')) for i in range(common_config['max_dom'])]
	if not check_files(expected_files) or args.force:
		run('rm -f wrfout_*')
		try:
			proc = run(f'mpiexec -np {args.np} ./wrf.exe', bg=True)
	...
	run(f'ls -l {wrf_root}/run/wrfout_*')
...
if __name__ == '__main__':
	...
	run_wrf(args.wrf_root, args.wps_root, config, args)
