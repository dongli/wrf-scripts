...
def parse_domain(config):
	...
	config['dx'] = [config['dx']]
	config['dy'] = [config['dy']] if 'dy' in config else config['dx']
	config['i_parent_start'] = [1]
	config['j_parent_start'] = [1]

	if 'ref_lat' in config and 'ref_lon' in config and 'truelat1' in config and 'truelat2' in config and 'stand_lon' in config:
		pass
	else:
		# Coarse domain
...
def parse_config(config_json):
	...
	if not 'time_step' in common_config:
		if common_config['dx'][0] >= 30000:
 			common_config['time_step'] = 120
		elif 25000 <= common_config['dx'][0] < 30000:
			common_config['time_step'] = 120
		elif 20000 <= common_config['dx'][0] < 25000:
			common_config['time_step'] = 120
		elif 15000 <= common_config['dx'][0] < 20000:
			common_config['time_step'] = 90
		elif 10000 <= common_config['dx'][0] < 15000:
			common_config['time_step'] = 60
		elif 5000 <= common_config['dx'][0] < 10000:
			common_config['time_step'] = 30
		elif 2500 <= common_config['dx'][0] >= 5000:
